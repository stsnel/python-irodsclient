...
class TestQuery(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.obj_name = 'test1'
        self.case_sensitive_obj_name = 'caseSENSITIVEobject'
        self.obj_path = '{coll_path}/{obj_name}'.format(**vars(self))
        self.case_sensitive_obj_path = '{coll_path}/{case_sensitive_obj_name}'.format(
            **vars(self))
        ...
        self.csobj = self.sess.data_objects.create(
            self.case_sensitive_obj_path)
    ...
    def test_files_query_case_sensitive(self):
        ...
        result1 = self.sess.query(DataObject.name).filter(
            DataObject.name == self.case_sensitive_obj_name).all()
        assert result1.has_value(self.case_sensitive_obj_name)
        assert len(result1) == 1

        result2 = self.sess.query(DataObject.name).filter(
            DataObject.name == str.lower(self.case_sensitive_obj_name)).all()
        assert not result2.has_value(self.case_sensitive_obj_name)
        assert len(result2) == 0

        result3 = self.sess.query(DataObject.name).filter(
            DataObject.name == str.upper(self.case_sensitive_obj_name)).all()
        assert not result3.has_value(self.case_sensitive_obj_name)
        assert len(result3) == 0

        # Wildcard filter tests

        search_expression = "%{}%".format(self.case_sensitive_obj_name[1:-1])

        result4 = self.sess.query(DataObject.name).filter(
            Like(DataObject.name, search_expression)).all()
        assert result4.has_value(self.case_sensitive_obj_name)
        assert len(result4) == 1

        result5 = self.sess.query(DataObject.name).filter(
            Like(DataObject.name, str.lower(search_expression))).all()
        assert not result5.has_value(self.case_sensitive_obj_name)
        assert len(result5) == 0

        result6 = self.sess.query(DataObject.name).filter(
            Like(DataObject.name, str.upper(search_expression))).all()
        assert not result6.has_value(self.case_sensitive_obj_name)
    ...
    def test_files_query_case_insensitive(self):
        ...
        upper_case_name = str.upper(self.case_sensitive_obj_name)
        ...
        result2 = self.sess.query(DataObject.name, case_sensitive=False).filter(
            DataObject.name == str.lower(self.case_sensitive_obj_name)).all()
        assert result2.has_value(self.case_sensitive_obj_name)
        assert len(result2) == 1

        result3 = self.sess.query(DataObject.name, case_sensitive=False).filter(
            DataObject.name == str.upper(self.case_sensitive_obj_name)).all()
        assert result3.has_value(self.case_sensitive_obj_name)
        assert len(result3) == 1

        # Single wildcard filter tests
        ...
        result5 = self.sess.query(DataObject.name, case_sensitive=False).filter(
            Like(DataObject.name, str.lower(search_expression))).all()
        assert result5.has_value(self.case_sensitive_obj_name)
        assert len(result5) == 1

        result6 = self.sess.query(DataObject.name, case_sensitive=False).filter(
            Like(DataObject.name, str.upper(search_expression))).all()
        assert result6.has_value(self.case_sensitive_obj_name)
        ...
        assert result7.has_value(self.case_sensitive_obj_name)
        ...
        assert result8.has_value(self.case_sensitive_obj_name)
        ...
        assert result9.has_value(self.case_sensitive_obj_name)
    ...
    def test_not_like_operator__issue_443(self):
        ...
        query = self.sess.query(DataObject).filter( NotLike(DataObject.name, name_search_pattern),
                                                    DataObject.collection_id == self.coll.id)
        self.assertEqual(2, len(list(query)))
